...
//
//
//  Edited by luca3104 on 2018/02/11.
...
private func randombytes(_ r:inout [UInt8], len:Int)
{
...
private func crypto_verify_32(_ x:[UInt8], _ y:[UInt8]) -> Bool
{
...
// @param pk: 32bytes
// @param sk: 32bytes
func generate_keypair() -> (pk:[UInt8], sk:[UInt8])
{
...
// @param sk : secret key
func crypto_pk(_ sk:[UInt8]) -> [UInt8]
{
    assert(sk.count == 32)
    var scsk = sc()
    var gepk = ge()
...
func isvalid_keypair(_ pk:[UInt8], _ sk:[UInt8]) -> Bool
{
...
// m: message
// return : R + m + S
func generate_sign(_ sm:inout [UInt8], _ m:[UInt8], _ skpk:[UInt8])
{
